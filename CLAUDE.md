...
- **LlamaIndex stack** in `backend/pyproject.toml` is on `0.13.x`, declared as compatible ranges (`llama-index-core>=0.13.0,<0.14.0`, etc.); `uv.lock` pins the resolved versions. The API churned hard across the 0.11→0.13 majors, so jumping a major can require code changes in `ModelsManager.py`, `vector_store_manager.py`, and `query_engine.py`. The Gemini provider uses **`llama-index-llms-google-genai`** (`GoogleGenAI`, bare model id like `gemini-2.5-pro`) — the older `llama-index-llms-gemini` is deprecated and was removed.
